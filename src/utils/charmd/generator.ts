  ...
  switch (node.type) {
    ...
    case "blockquote":
      return node.children
        ?.map((child: Node) =>
          generator(child, node, options)
            ?.split("\n")
            .map((l: string, i, a) => (i !== a.length - 1 ? colors.gray(colors.italic(l)) : l))
            .map((l, i, a) => (i !== a.length - 1 && l.trim() ? "┃ " : "") + l)
            .join("\n"),
        )
        .join("")
        ...
        return (
          node.children
            ?.map((child, i: number) => {
              ...
              return (
                (icon(i) + generator(child, node, options))
                  ?.replace(/\n$/, "")
                  .split("\n")
                  .map((l, i) => tabForList + (i ? tabForMultiline + l : l))
                  .join("\n") + "\n"
              )
            })
            .join("")
            .split("\n")
            .map((l: string) => l.replace(tabForList, ""))
            .join("\n") ?? ""
        )
      }
