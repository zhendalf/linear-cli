...
export const updateCommand = new Command("update")
  ...
  .option("--status <status>", "New status (planned, active, completed, paused)")
  ...
  .action(async (initiativeId: string, options) => {
    ...
    const noFlagsProvided =
      !name && !description && !status && !owner && !targetDate && !colorHex && !icon
    ...
    if (noFlagsProvided && isInteractive) {
      ...
      const newStatus = await select({
        ...
        default:
          currentStatusIndex >= 0 ? INITIATIVE_STATUSES[currentStatusIndex].value : undefined,
      })
