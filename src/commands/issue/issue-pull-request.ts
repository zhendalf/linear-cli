...
export const pullRequestCommand = new Command("pull-request")
  ...
  .alias("pr")
  .option("--base <branch>", "The branch into which you want your code merged")
  .option("--draft", "Create the pull request as a draft")
  .option(
    "-t, --title <title>",
    "Optional title for the pull request (Linear issue ID will be prefixed)",
  )
  .option("--web", "Open the pull request in the browser after creating it")
  .option("--head <branch>", "The branch that contains commits for your pull request")
  .argument("[issueId]")
