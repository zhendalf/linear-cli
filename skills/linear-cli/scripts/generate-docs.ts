...
async function main() {
  ...
  const versionResult = await run([...LINEAR_CMD, "--version"])
  ...
  try {
    entries = await readdir(REFERENCES_DIR, { withFileTypes: true })
  } catch {
    entries = []
  }
  ...
  await writeFile(join(REFERENCES_DIR, "commands.md"), indexContent, "utf8")
  ...
  await writeFile(SKILL_MD, skillContent, "utf8")
...
function generateCommandsSection(commands: CommandInfo[]): string {
  return commands.map((cmd) => flattenCommandPaths(cmd).join("\n")).join("\n\n")
}
...
async function generateSkillMd(commands: CommandInfo[]): Promise<string> {
  const template = await readFile(SKILL_TEMPLATE, "utf8")
  return template
